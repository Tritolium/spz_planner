            ...
            {attendences.map(event => {
                return(<Fragment key={`row_${event.Event_ID}`}>
                    ...
                    {event.Attendences.map((attendence, index) => {
                        ...
                        return(<span key={attendence.Fullname + event.Event_ID} className={className}><Zusage attendence={attendence.Attendence} plusone={attendence.PlusOne} prediction={attendence.Prediction} theme={theme} credible={attendence.Credible}/></span>)
                    })}
                </Fragment>)
            })}
        </StyledOverviewTable>)

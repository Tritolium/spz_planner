...
import { Alert, Blank, Check, Delayed, Deny, Unregistered } from "./Terminzusage";
...
export const AttendenceInput = ({ event, attendence, onClick, theme }) => {
    ...
    const onDelayedClick = useCallback(async () => {
        updateAttendence(ATTENDENCE_DELAYED)
    }, [updateAttendence])
    ...
    return(
        <StyledAttendenceInput className="AttendenceInput" active={`${active}`}>
            ...
            <Button className={`${active ? 'active_delayed': ''} ${att === ATTENDENCE_DELAYED || active ? 'selected': 'not_selected'}`} attendence={ATTENDENCE_DELAYED} callback={onDelayedClick} theme={theme}/>
...
const Button = ({ attendence, callback, className, theme }) => {
    ...
    case ATTENDENCE_DELAYED:
        return(<Delayed className={className} callback={callback} theme={theme}/>)
    }
}
